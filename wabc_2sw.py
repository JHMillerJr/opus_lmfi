...
def metric(sample1, sample2, L):
    ...
    #> normal unperturbed data (to get mu)
    swd_mean = swd(sample1, sample2, n_projections=L)
    
    #> subtracting mu (to get std)
    swd_std = swd(sample1-np.mean(sample1), 
                  sample2-np.mean(sample2), 
                  n_projections=L)
    
    #> subtract mu, dividing by the std (to get skew)
    swd_skew = swd((sample1-np.mean(sample1))/np.std(sample1), 
                  (sample2-np.mean(sample2))/np.std(sample2),
                  n_projections=L)
    
    return swd_mean, swd_std, swd_skew
...
def cornerPlot(truth, num_post=1000):
    ...
    #> globals
    global outFile_obs, outFile_mock, num_dists
    ...
    #> plotting posterior
    fig = corner.corner(posterior[:, :-num_dists], labels=labels,
...
def post(num_post=1000, resample_perc=(2/3),
    ...
    #> globals
    global outFile_mock, bounds, num_dists
    lb, ub = bounds
    ...
    for num in range(num_resamples):
        ...
        #> collecting moments
        tg_means, tg_stds = [], []
        for i in range(len(mock_data[0]) - num_dists):
...
def checkConverg(num_post=1000, resample_perc=(2/3),
    ...
    #> globals
    global outFile_mock, d, num_dists
    ...
    for num in range(num_resamples):
        ...
        #> checking posterior
        posterior = subset[np.argsort(subset[:, -num_dists])][:num_post]
...
def abc_single(args):
    ...
    swd_mean, swd_std, swd_skew = metric(obs_sample, mock_sample, num_slices)
    swd_ttl = swd_mean + swd_std + swd_skew

    return np.hstack([mu, sigma, r, swd_mean, swd_std, swd_skew, swd_ttl])
...
global d, bounds, outFile_obs, outFile_mock, convergFile, num_dists
d = 2
bounds = (-10, 10)
...
if __name__ == '__main__':
    ...
    truth, obs_mu, obs_sigma, obs_r, obs_sample = obs(
        create=True, num_samples=num_samples)

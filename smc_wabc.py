...
def MCKernel(args):
    ...
    #> globals
    global num_parts, num_mock_samples, num_slices
    
    #> unpacking
    theta, dist, epsilon, obs_sample = args
    ...
    while len(L) < num_hits_r and max_tries > 0:
        ...
        prime_dist_i = swd(X_s=obs_sample, 
                           X_t=prime_sample_i, 
                           n_projections=num_slices)
...
def initStep():
    ...
    epsilon_1 = np.percentile(dists0, 50)
    
    return theta0, dists0, epsilon_1


#> repeated step of SMC
def repStep(theta_t_1, dist_t_1, epsilon_t):
    ...
    weights_t = np.where(dist_t_1, dist_t_1 <= epsilon_t, 1.0)
    ...
    ancestor_thetas = theta_t_1[drawn_indices]
    ancestor_distances = dist_t_1[drawn_indices]

    #(C)> rejuvenation: sample new points from markov kernel
    theta_k_t, dist_k_t = [], []
    
    workers = max(1, mp.cpu_count() - 1)
    args = [(ancestor_thetas, ancestor_distances, epsilon_t)]
    
    args = list(zip(ancestor_thetas, ancestor_distances, [epsilon_t] * num_parts, [obs_sample] * num_parts))
    
    with mp.Pool(processes=workers) as pool:
        for tprime, dprime in tqdm(pool.imap(MCKernel, args), total=num_parts):
    ...
    return theta_k_t, dist_k_t, epsilon_t_1
...
def smc(num_gens):
    ...
    theta, dists, epsilon = initStep()
    
    #> repeated steps, main generations fn
    for i in tqdm(range(num_gens)):
        theta, dists, epsilon = repStep(theta, dists, epsilon)
        
        #for r in tqdm(pool.imap(abc_single, args), total=num_samples):
        #    np.savetxt(f, r.reshape(1, -1))
    
    return theta, dists, epsilon
...
if __name__ == '__main__':
    ...
    #> abc-smc parameters
    num_generations = 50      # num of generations
    # epsilon_schedule = np.linspace(2.0, 0.05, num_generations)  # decreasing tolerances
    
    post_theta, dists, epsilon = smc(num_gens=num_generations)
    # post_mu, post_sigma, post_r = unpack(post_theta)
    
    fig, axes = plt.subplots(1,5,figsize=(6*5,6))
    
    for i, ax in enumerate(axes.flat):
        
        ax.hist(post_theta[:,i])
        
    plt.show()
    
    #> posterior sample
    # post_sample = model(post_theta, num_samples)
    
    #> plots
    # bivarPlot(obs_sample, post_sample)
    # cornerPlot(obs_theta, num_post=min(1000, num_particles))
